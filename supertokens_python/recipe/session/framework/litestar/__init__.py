...
def verify_session(
    ...
    async def func(request: Request[Any, Any, Any]) -> Union[SessionContainer, None]:
        nonlocal user_context
        base_req = LitestarRequest(request)
...
async def session_exception_handler(
    ...
    """Litestar exceptional handler for errors raised by Supertokens SDK when not using middleware

    Usage: `app.add_exception_handler(SuperTokensError, st_exception_handler)`
    """
    base_req = LitestarRequest(request)
    base_res = LitestarResponse(Response(content={}))
    ...
    if isinstance(result, LitestarResponse):
        body = json.loads(bytes(result.response.content))
        return Response(body, status_code=result.response.status_code)
