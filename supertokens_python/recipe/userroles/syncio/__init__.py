...
from supertokens_python.recipe.userroles.interfaces import (
    AddRoleToUserResult, CreateNewRoleOrAddPermissionsResult, DeleteRoleResult,
    GetAllRolesResult, GetPermissionsForRoleResult, GetRolesForUserResult,
    GetRolesThatHavePermissionResult, GetUsersThatHaveRoleResult,
    RemovePermissionsFromRoleResult, RemoveUserRoleResult)


def add_role_to_user(user_id: str, role: str,
...
def remove_user_role(user_id: str, role: str, user_context: Dict[str, Any]) -> RemoveUserRoleResult:
...
def get_roles_for_user(user_id: str, user_context: Dict[str, Any]) -> GetRolesForUserResult:
    from supertokens_python.recipe.userroles.asyncio import get_roles_for_user
    return sync(get_roles_for_user(user_id, user_context))


def get_users_that_have_role(role: str, user_context: Dict[str, Any]) -> GetUsersThatHaveRoleResult:
    from supertokens_python.recipe.userroles.asyncio import \
        get_users_that_have_role
    return sync(get_users_that_have_role(role, user_context))


def create_new_role_or_add_permissions(role: str, permissions: List[str],
...
def get_permissions_for_role(role: str, user_context: Dict[str, Any]) -> GetPermissionsForRoleResult:
    from supertokens_python.recipe.userroles.asyncio import \
        get_permissions_for_role
    return sync(get_permissions_for_role(role, user_context))


def remove_permissions_from_role(role: str, permissions: List[str],
...
def get_roles_that_have_permission(permission: str,
...
def delete_role(role: str, user_context: Dict[str, Any]) -> DeleteRoleResult:
    from supertokens_python.recipe.userroles.asyncio import delete_role
    return sync(delete_role(role, user_context))


def get_all_roles(user_context: Dict[str, Any]) -> GetAllRolesResult:
