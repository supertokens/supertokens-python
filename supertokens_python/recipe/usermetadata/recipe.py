...
class UserMetadataRecipe(RecipeModule):
    ...
    def is_error_from_this_recipe_based_on_instance(
        ...
        return isinstance(err, SuperTokensError) and (isinstance(err, SuperTokensUserMetadataError))
    ...
    def init(override: Union[InputOverrideConfig, None] = None):
        def func(app_info: AppInfo):
            if UserMetadataRecipe.__instance is None:
                UserMetadataRecipe.__instance = UserMetadataRecipe(UserMetadataRecipe.recipe_id, app_info, override)
                return UserMetadataRecipe.__instance
            raise Exception(None, 'Usermetadata recipe has already been initialised. Please check your code for bugs.')
