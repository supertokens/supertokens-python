...
class JWTRecipe(RecipeModule):
    ...
    def get_apis_handled(self) -> List[APIHandled]:
        return [APIHandled(method='get', path_without_api_base_path=NormalisedURLPath(GET_JWKS_API), request_id=GET_JWKS_API,
                           disabled=self.api_implementation.get_JWKS_GET is None)]
    ...
    async def handle_api_request(self, request_id: str, request: BaseRequest, path: NormalisedURLPath, method: str,
        ...
        return await get_JWKS(self.api_implementation, options)
    ...
    def is_error_from_this_recipe(self, err):
        return isinstance(err, SuperTokensError) and err.from_recipe == self.get_recipe_id()
