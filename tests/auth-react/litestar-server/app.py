...
# App will be initialized at the end after all route handlers are defined
...
async def add_user_to_tenant(request: Request) -> Response:
    ...
    if isinstance(core_resp, AssociateUserToTenantOkResult):
        return Response(
            content={
                "status": "OK",
                "wasAlreadyAssociated": core_resp.was_already_associated,
            }
        )
...
app = Litestar(
    ...
    openapi_config=OpenAPIConfig(
        title="SuperTokens Litestar Demo",
        version="1.0.0",
        path="/docs",
    ...
    middleware=[
        create_supertokens_middleware(),
        # DefineMiddleware(log_response_middleware),  # Uncomment for response logging
    ],
