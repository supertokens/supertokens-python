...
def custom_decorator_for_test():  # type: ignore
    def session_verify_custom_test(f):  # type: ignore
        @wraps(f)  # type: ignore
        async def wrapped_function(request: HttpRequest, *args: Any, **kwargs: Any):  # type: ignore
            Test.increment_attempted_refresh()
            try:
                value: HttpResponse = await f(request, *args, **kwargs)  # type: ignore
...
async def login(request: HttpRequest):
    if request.method == "POST":
        ...
        session_ = await create_new_session(request, "public", RecipeUserId(user_id))
...
async def after(request: HttpRequest):
    if request.method == "POST":
        return HttpResponse("")
    else:
        return send_options_api_response()
...
async def setup_st(request: HttpRequest):
    global last_set_enable_jwt
    global last_set_enable_anti_csrf
    data = json.loads(request.body)
    ...
    config(
        ...
        enable_jwt=data.get("enableJWT"),
        jwt_property_name=data.get("jwtPropertyName"),
    )

    last_set_enable_anti_csrf = data.get("enableAntiCsrf")
    last_set_enable_jwt = data.get("enableJWT")
    return HttpResponse("")
