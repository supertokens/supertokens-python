...
async def test_get_users_pagination():
    ...
    assert [user.email for user in response.users] == [f"dummy{i}@gmail.com" for i in range(5)][::-1]

    # Get only the oldest user
    response = await get_users_oldest_first(limit=1)
    assert [user.email for user in response.users] == ["dummy0@gmail.com"]

    # Test pagination
    response = await get_users_oldest_first(limit=1, pagination_token=response.next_pagination_token)
