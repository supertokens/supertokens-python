...
@mark.asyncio
async def test_login_logout(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
...
@mark.asyncio
async def test_login_info(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
...
@mark.asyncio
async def test_login_handle(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
...
@mark.asyncio
async def test_login_refresh_error_handler(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
...
@mark.asyncio
async def test_custom_response(driver_config_client: TestClient[Litestar]):
...
@mark.asyncio
async def test_optional_session(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
...
@mark.parametrize(
    ...
    [
        ...
        "/api",
        "api",
        # Don't pass "/" as litestar_root_path. You'll get unexpected behaviour
        # because api_base_path will be "//auth"
    ],
)
def test_litestar_root_path(litestar_root_path: str):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
    ...
    # Test with root_path
    if litestar_root_path.startswith("/"):
...
async def test_should_clear_all_response_during_refresh_if_unauthorized(
    driver_config_client: TestClient[Litestar], token_transfer_method: str
):
...
async def test_session_with_legacy_refresh_token_and_unauthorized_should_clear_legacy_token(
    driver_config_client: TestClient[Litestar],
):
    init(
        **get_st_init_args(
            url=get_new_core_app_url(),
...
@mark.asyncio
async def test_search_with_email_t(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
...
@mark.asyncio
async def test_search_with_email_iresh(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
...
@mark.asyncio
async def test_search_with_phone_plus_one(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
...
@mark.asyncio
async def test_search_with_provider_google(driver_config_client: TestClient[Litestar]):
    init(
        supertokens_config=SupertokensConfig(get_new_core_app_url()),
        app_info=InputAppInfo(
            app_name="SuperTokens Demo",
